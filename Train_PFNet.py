...
parser.add_argument('--netG', default='./Checkpoint/point_netG100.pth', help="path to netG (to continue training)")
parser.add_argument('--netD', default='./Checkpoint/point_netD100.pth', help="path to netD (to continue training)")
    ...
    if opt.D_choose == 1:
        for epoch in range(resume_epoch,opt.niter):
            ...
            for i, data in enumerate(dataloader, 0):
                ...
                np.savetxt('./post_set' + '.txt', torch.cat([input_cropped1[0,:,:],
                                                             torch.squeeze(fake, 1)[0,:,:]],
                                                            dim=0).cpu().detach().numpy(), fmt="%f,%f,%f")
                ...
                np.savetxt('./fake' + '.txt', torch.squeeze(fake,1)[0, :, :].cpu().detach().numpy(),
                           fmt="%f,%f,%f")  # input_A
                np.savetxt('./real_center' + '.txt', torch.squeeze(real_center,1)[0, :, :].cpu().detach().numpy(),
                           fmt="%f,%f,%f")  # input_B
